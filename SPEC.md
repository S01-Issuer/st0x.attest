...
7 and the words from items 8 to 13. It is one expression a mint admin
 ...
 * attestors does not catch a price they all got wrong. */
:ensure(
  greater-than-or-equal-to(lead-price() min-price)
  "Price below minimum"
),
:ensure(
  less-than-or-equal-to(lead-price() max-price)
  "Price above maximum"
),
...
14. The parties are uncoordinated in that they do not coordinate requests
...
15. It does not need to be fully decentralised. What is needed is orthogonal
...
16. The operators are a pool, with the mint collecting a threshold M smaller
...
17. The lead is mandatory. S01 is the issuer, the entity legally issuing the
    share tokens and minting them. The other entities are not legally
    responsible.
18. To reach the signing threshold and make it possible to raise the mint cap,
...
19. The weighting produced by the Rainlang in section 2 is what increases the
    leaky bucket, in place of global and per-token mappings holding raw token
    amounts.
20. The per-token mappings are then not needed. Only a single global limit per
    ...
    different attestors.
21. Deploying different algorithms and weightings is then something governance
    handles, without rewriting the smart contract logic.
22. The leaky bucket logic converts from Rain fixed point to Rain Floats.
23. Two storage slots is acceptable. Typical mints can be large — an incoming
    OTC mint requesting $2 million in a single mint — so the gas difference
    between one slot and two is irrelevant.
24. Zero values are not accepted. The leaky bucket library already rejects
    them.
