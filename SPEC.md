...
7 and the words from items 8 to 12. It is one expression a mint admin
...
13. The parties are uncoordinated in that they do not coordinate requests
...
14. It does not need to be fully decentralised. What is needed is orthogonal
...
15. The operators are a pool, with the mint collecting a threshold M smaller
...
16. The lead is mandatory. S01 is the issuer, the entity legally issuing the
    share tokens and minting them. The other entities are not legally
    responsible.
17. To reach the signing threshold and make it possible to raise the mint cap,
...
18. The weighting produced by the Rainlang in section 2 is what increases the
    leaky bucket, in place of global and per-token mappings holding raw token
    amounts.
19. The per-token mappings are then not needed. Only a single global limit per
    ...
    different attestors.
20. Deploying different algorithms and weightings is then something governance
    handles, without rewriting the smart contract logic.
21. The leaky bucket logic converts from Rain fixed point to Rain Floats.
22. Two storage slots is acceptable. Typical mints can be large — an incoming
    OTC mint requesting $2 million in a single mint — so the gas difference
    between one slot and two is irrelevant.
23. Zero values are not accepted. The leaky bucket library already rejects
    them.
