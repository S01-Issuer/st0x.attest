...
15. The parties are uncoordinated in that they do not coordinate requests
...
16. It does not need to be fully decentralised. What is needed is orthogonal
...
17. The operators are a pool, with the mint collecting a threshold M smaller
...
18. The lead is mandatory. S01 is the issuer, the entity legally issuing the
    share tokens and minting them. The other entities are not legally
    responsible.
19. To reach the signing threshold and make it possible to raise the mint cap,
...
20. The weighting produced by the Rainlang in section 2 is what increases the
    leaky bucket, in place of global and per-token mappings holding raw token
    amounts.
21. The per-token mappings are then not needed. Only a single global limit per
